...
def load_path_loss_with_detail_dataset(input_dir, model_type='RNN', num_workers=4, batch_size=128,
    ...
    pathloss_valid_dataset = PathLossWithDetailDataset(input_data=valid_data,
                                                       model_type=model_type)
    pathloss_train_dataloader = DataLoader(pathloss_train_dataset, batch_size=batch_size, shuffle=True,
                                           num_workers=num_workers)
    pathloss_test_dataloader = DataLoader(pathloss_test_dataset, batch_size=batch_size, shuffle=False,
                                          num_workers=num_workers)
    pathloss_valid_dataloader = DataLoader(pathloss_valid_dataset, batch_size=batch_size, shuffle=False,
                                           num_workers=num_workers)
